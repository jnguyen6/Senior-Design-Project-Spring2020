...
class Patient(db.Model):
    ...
    OptOut = db.Column(db.String(50), unique=False, nullable=True)

    def __repr__(self):
        return f'<Account ID={self.accountId} Gender={self.gender} Birth Year={self.birth_year} Address Zip={self.address_zip} Family Income={self.family_income} Bill Amount={self.bill_amount} Opt-out Status={self.OptOut}'